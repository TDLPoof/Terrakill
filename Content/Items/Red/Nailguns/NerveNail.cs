...
public class NerveNail : ModProjectile
{
    int bounces;
    ...
    public override void AI()
    {
        if (!Main.dedServ)
        {
            Dust d = Dust.NewDustDirect(Projectile.Center, 1, 1, DustID.Clentaminator_Red, Scale: 0.5f);
            d.noGravity = true;
        ...
        {
            if (p.active && p.type == ModContent.ProjectileType<Blue.Nailguns.Magnet>() && p.Distance(Projectile.position) < 150)
            {
        ...
        foreach (Projectile p in Main.projectile)
        {
            if (p.active && p.type == ModContent.ProjectileType<NerveMarker>() && p.Distance(Projectile.position) < 250d)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    foreach (NPC npc in Main.npc)
                    {
                        if (npc.active && !npc.friendly && !npc.dontTakeDamage && npc.Hitbox.Contains(positionOnLine.ToPoint()))
                        {
                            Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), npc.Center, Vector2.Zero,
                                ModContent.ProjectileType<NerveDamage>(), 20, 0, Projectile.owner);
                        }
                    }
                }
            }
