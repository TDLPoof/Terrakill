...
namespace Terrakill.Content.Items.Red.Railcannons;

public class MauriceRailshot : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        {
            MakeADust(DustID.RedTorch, 2f, 0);
            MakeADust(DustID.Torch, 2f, 0);
        }
        ...
        foreach (Projectile p in Main.projectile)
        {
            ...
            Explode(400, 2000, DustID.RedTorch, DustID.Torch);
            p.Kill();
            Projectile.Kill();
        }
    ...
    public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
    {
        ...
        Explode(250, 200, DustID.RedTorch, DustID.Torch);
    }

    public override bool OnTileCollide(Vector2 oldVelocity)
    {
        Explode(250, 600, DustID.RedTorch, DustID.Torch);
        return true;
    }
    ...
    public void CoinBounce(Projectile originalCoin)
    {
        ...
        List<NPC> available = new List<NPC>();
        ...
        else
        {
            ...
            {
                if (Projectile.Center.Distance(available[i].Center) < Projectile.Center.Distance(npc.Center))
                {
                    npc = available[i];
                }
            }
            Projectile.velocity = Projectile.Center.DirectionTo(npc.Center) * Projectile.velocity.Length();
            Projectile.damage = (int)MathF.Round((Projectile.damage * 1.2f) + (Projectile.Center.Distance(npc.Center) / 3f));
            return;
        }
    }
