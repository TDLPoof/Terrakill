...
public class SlabSharpshooterSuper : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        foreach (Projectile p in Main.projectile)
        {
            ...
            if (p.type == ModContent.ProjectileType<SlabSharpshooterSpin>()) continue;
            if (!p.active) continue;
            if (p.Center.Distance(Projectile.Center) < 10)
            {
                Explode(100, 200, DustID.WhiteTorch, DustID.RedTorch);
                p.Kill();
                Projectile.penetrate--;
            }
        }
