...
public class AltElectricRailshot : ModProjectile
{
    ...
    public void MakeADust(int type, float scale = 1f, float velocityMult = 1f, bool velocityChange = false)
    {
        ...
        if (velocityChange) d.velocity += Main.rand.NextVector2CircularEdge(1.1f, 1.1f);
    }
    ...
    public override void AI()
    {
        ...
        {
            if (Main.rand.NextBool()) MakeADust(DustID.Flare_Blue, 1.5f, 0, true);
            if (Main.rand.NextBool()) MakeADust(DustID.BlueTorch, 1f, 0);
            if (Main.rand.NextBool()) MakeADust(DustID.Clentaminator_Cyan, 0.75f, 0);
        }
