...
public class Rocket : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Projectile == Main.player[Projectile.owner].GetModPlayer<RocketRiding>().rocketRide)
        {
            if (fallingTicks++ > 180 && ModContent.GetInstance<ServerConfigurations>().developerKey.ToLower() != "rocketfun")
            {
                fallingAmt += 0.03f;
                Projectile.velocity *= 0.92f;
            }
        }
        Projectile.velocity.Y += fallingAmt;
    ...
    {
        if (Main.player[Projectile.owner].GetModPlayer<RocketRiding>().rocketRide != null)
        {
            //Main.player[Projectile.owner].position -= Projectile.velocity;
            Main.player[Projectile.owner].GetModPlayer<RocketRiding>().rocketRide = null;
        }
        base.Kill(timeLeft);
    }
}
