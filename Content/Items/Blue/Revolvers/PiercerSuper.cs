...
public class PiercerSuper : ModProjectile
{
    ...
    {
        if (Main.rand.NextBool(2) && !Main.dedServ && Projectile.Distance(Main.player[Projectile.owner].position) < Main.screenWidth * 0.75f)
        {
            Dust d = Dust.NewDustDirect(Projectile.Center - new Microsoft.Xna.Framework.Vector2(1, 1), 2, 2, DustID.Clentaminator_Cyan);
            d.noGravity = true;
        ...
        {
            if (p.type != ModContent.ProjectileType<Shotguns.CoreBomb>()) continue;
            if (p.Distance(Projectile.position) > 40) continue;
            Explode(250, 400, DustID.Torch, DustID.Clentaminator_Cyan);
            p.Kill();
