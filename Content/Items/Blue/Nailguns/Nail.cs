...
public class Nail : ModProjectile
{
    ...
    public override void AI()
    {
        if (!Main.dedServ)
        {
            Dust d = Dust.NewDustDirect(Projectile.Center, 1, 1, DustID.Clentaminator_Cyan, Scale: 0.5f);
            d.noGravity = true;
        ...
        {
            if (p.type == ModContent.ProjectileType<Magnet>() && p.Distance(Projectile.position) < 150)
            {
