...
public class SlabSignalBullet : ModProjectile
{
    ...
    public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
    {
        ...
        foreach (Projectile p in Main.projectile)
        {
            if (p.type == ModContent.ProjectileType<ZeGold.Revolvers.SignalTrap>() && p.Hitbox.Intersects(Projectile.Hitbox)) Projectile.damage = (int)MathF.Round(Projectile.damage * 1.5f);
        }
        if (target.GetGlobalNPC<ZeGold.Revolvers.TrapManager>().trap != null)
        {
    ...
    public void CoinBounce(Projectile originalCoin)
    {
        foreach (Projectile p in Main.projectile)
        {
            ...
            if (p.type == ModContent.ProjectileType<ZeGold.Revolvers.SignalTrap>()
